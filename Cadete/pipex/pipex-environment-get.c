...
/*   Updated: 2023/06/26 08:33:32 by mister-code      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void path_split(t_pipe *set, char *str)
{
    ...
    while (buffer)
    {
        ...
        if (remain)
            chained_next_last(&set->path, chained_push(path_allocate(buffer, remain - buffer)));
        if (buffer && remain)
            buffer = remain + 1;
        if (!remain)
            break ;
    ...
    if (buffer)
        chained_next_last(&set->path, chained_push(path_allocate(buffer, ft_strlen(buffer) + 1)));
}
