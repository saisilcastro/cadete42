...
t_image	image_of_create_start(void *plugin, t_vi2d size)
{
	...
	set.addr = mlx_get_data_addr(set.buffer, &set.bpp, &set.length, &set.endian);
...
void image_of_start(t_image *set, t_vi2d size, t_pixel color)
{
	if (!set)
		return ;
	set->buffer = NULL;
...
t_image  *image_of_set(void *plugin, t_vi2d size)
{
	t_image	*get = (t_image *)malloc(sizeof(t_image));
	get->size->x = size.x;
	get->size->y = size.y;
	get->buffer = mlx_new_image(plugin, get->size->x, get->size->y);
	get->addr = mlx_get_data_addr(get->buffer, &get->bpp, &get->length, &get->endian);
	get->color = pixel_rgba_local(255, 255, 255, 255);
	return (get);
}

t_image	*image_of_buffer(void *buffer, t_vi2d size)
{
	t_image	*get = (t_image *)malloc(sizeof(t_image));
	get->size->x = size.x;
	get->size->y = size.y;
