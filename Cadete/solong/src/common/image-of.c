...
/*   Updated: 2023/07/20 09:21:29 by mister-code      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_image	*image_of_push(int id, void *buffer, t_vi2d size)
{
	t_image	*set;
...
t_status	image_of_id(t_image *set, int id)
{
	if (!set || set->id != id)
		return (Off);
	return (On);
...
unsigned int	image_color_int(t_image *data, int x, int y)
{
	if (!data)
		return (0);
...
void	image_pixel_to(t_image *data, t_pixel *px, t_vi2d pt)
{
	...
	*(unsigned int *)dest = (px->a << 24 | px->r << 16 | px->g << 8 | px->b);
}
