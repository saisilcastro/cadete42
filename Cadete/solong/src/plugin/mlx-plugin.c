...
/*   Updated: 2023/07/03 17:51:38 by mister-code      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_STATUS		mlx_plugin_start(t_machine *set)
{
	...
	w = set->size->x;
	h = set->size->y;
	((t_mlx_plugin*)set->plugin)->window = mlx_new_window(lead, w, h, set->title);
	if (!set->plugin)
		return (Off);
	return (On);
}
...
void		mlx_start_event(t_machine *set)
{
	...
	mlx_hook(plugin->window, ON_MOUSEDOWN, MOUSE_PRESS_MASK, mlx_mouse_press, set);
	mlx_hook(plugin->window, ON_MOUSEUP, MOUSE_RELEASE_MASK, mlx_mouse_release, set);
